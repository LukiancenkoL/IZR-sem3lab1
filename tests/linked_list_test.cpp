 ...
 * @brief Unit tests for LinkedList.
 * @version 0.1
 * @date 2025-11-18
 *  
 * @copyright Copyright (c) 2025
 *
 * Covers core list operations such as insertion, removal, popping,
 * and retrieval of elements, ensuring correct behavior of the
...
/**
 * @test Tests inserting elements at the front of the list.
 */
TEST(linked_list, push_front) {
    auto list = LinkedList<int32_t>{};
    list.push_front(5);
...
/**
 * @test Tests inserting elements at the back of the list.
 */
...
/**
 * @test Tests pop_back on non-empty and empty lists.
 */
...
/**
 * @test Tests pop_front on non-empty and empty lists.
 */
...
/**
 * @test Tests insertion at arbitrary indices, including
 *       insertion beyond the current size (append behavior).
 */
TEST(linked_list, insert) {
    ...
    // inserting at index > length => treated as push_back
    list.insert(999, 357);
    ASSERT_EQ(list.get_first(), 5);
    ASSERT_EQ(list.get(1), 14);
...
/**
 * @test Tests removing elements from different positions
 *       and verifies list integrity after removals.
 */
TEST(linked_list, remove) {
    ...
    // verify only odd numbers remain, in correct order
