...
TEST(sort, merge_sort) {
	auto merge_sort = MergeSort<int32_t>{};
	{
		...
		ASSERT_NO_THROW(list_empty.sort(merge_sort));
	}

	{
		auto list_one_element = ArrayList<int32_t>{};
		list_one_element.push_back(1);
		ASSERT_NO_THROW(list_one_element.sort(merge_sort));
	}

	{
		...
		list_two_equal_elements.push_back(1);
		ASSERT_NO_THROW(list_two_equal_elements.sort(merge_sort));
	}

	{
		...
		ASSERT_NO_THROW(list_many_equal_elements.sort(merge_sort));
...

	{
		...
		ASSERT_NO_THROW(list_two_sorted.sort(merge_sort));
...

	{
		...
		ASSERT_NO_THROW(list_two_unsorted.sort(merge_sort));
...

	{
		...
		ASSERT_NO_THROW(list_many_sorted.sort(merge_sort));
...

	{
		...
		ASSERT_NO_THROW(list_many_reverse.sort(merge_sort));
...

	{
		...
		ASSERT_NO_THROW(list_many_unsorted.sort(merge_sort));
